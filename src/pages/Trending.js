...
const Trending = ({ addType, userLoading, firebase }) => {
	const INITIAL_TYPE = {
		name: '',
		media: ''
	}
	...
	const handleImageUpload = async (typeId) => {
		await firebase
			.storage()
			.ref(`trends/${typeId}-${type.media.name}`)
			.put(type.media)
	}
	...
	const handleSubmit = async (e) => {
		try {
			...
			const {
				...
				'http://localhost:5001/floristeria-cra/us-central1/postTrend',
...
export default compose(
	firebaseConnect(),
	connect(mapStateToProps, { addType })
)(Trending)
